 ...
 * 球机方向控制：鼠标按下事件
 */
function control_direct(nDirect) {
	var step =  $("#step").val(); //获取步长
	...
	var strCameralId = dataMap.get(nWndNo);	// 球机通道id
	
	// 云台方向控制:通道ID、方向、步长、是否停止
	var nResult = obj.DPSDK_PtzDirection(strCameralId, nDirect, step, false);
	if (nResult == 0) {
		$("#myconsole").append("<div class='divide'>方向控制成功，步长="+step+"</div>");
	} else {
		$("#myconsole").append(
	...
	//控制成功后，停止方向控制
	nResult = obj.DPSDK_PtzDirection(strCameralId, nDirect, step, true);
	if (nResult == 0) {
